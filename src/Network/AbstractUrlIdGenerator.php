...
abstract class AbstractUrlIdGenerator implements UrlIdGenerator
{
    ...
    {
        if (strpos($url, self::PROTOCOL_HTTPS) > 0)
            return true;
        if (strpos($url, self::PROTOCOL_HTTP) > 0)
            return true;
    ...
    protected function normalizeUrl(string $url) : string
    {
        switch (true) {
            ...
            case
                ($this->hasProtocol($url) === false || $this->hasUrlParameter($url)):
                $url = $this->removePort($url);
                break;
        }
    ...
    protected function generateUniqueId($url) : string
    {
        return sprintf("%.0f",hexdec(md5($url)));
    }
